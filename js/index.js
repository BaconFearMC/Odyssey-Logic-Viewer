...
// Arrow cooldown — after using L/R (or the on-screen left/right arrows) to turn
// the page, both arrows are locked out for .5s and get a visible red pulse that
// fades bottom-to-top back to normal, so it's obvious when input is being ignored.
var arrowsOnCooldown = false;
var arrowCooldownTimeout = null;
var ARROW_COOLDOWN_MS = 500;
...
/* =========================================================
   ARROW COOLDOWN — after a page turn (L/R shoulder buttons, the
   Left/Right arrow keys, or on-screen left/right arrow buttons all
   funnel through customFunctionL/customFunctionR above), both arrows
   are locked out for ARROW_COOLDOWN_MS (.5s) and get a red pulse that
   sweeps in from the bottom and fades back out to normal over that
   same .5s, so it's visually obvious the arrows are briefly disabled.
   Injects its own <style> once so no separate CSS file edit is needed.
   Targets any element with the "left-arrow" or "right-arrow" class
   (the on-screen arrow buttons) - update ARROW_SELECTOR below if your
   markup uses a different class/id for them.
   ========================================================= */
var ARROW_SELECTOR = ".left-arrow, .right-arrow";

(function injectArrowCooldownStyle() {
    var style = document.createElement("style");
    style.textContent =
        "@keyframes arrowCooldownPulse {" +
        "  0%   { filter: brightness(1) sepia(0) saturate(1); }" +
        "  40%  { filter: brightness(0.75) sepia(0.6) saturate(3.5) hue-rotate(-15deg); }" +
        "  100% { filter: brightness(1) sepia(0) saturate(1); }" +
        "}" +
        ".arrow-cooldown {" +
        "  animation: arrowCooldownPulse " + ARROW_COOLDOWN_MS + "ms ease-out;" +
...
function startArrowCooldown() {
    ...
    arrowsOnCooldown = true;

    var arrows = document.querySelectorAll(ARROW_SELECTOR);
    for (var i = 0; i < arrows.length; i++) {
        arrows[i].classList.add("arrow-cooldown");
    }

    clearTimeout(arrowCooldownTimeout);
    arrowCooldownTimeout = setTimeout(function() {
        arrowsOnCooldown = false;
        var arrows = document.querySelectorAll(ARROW_SELECTOR);
        for (var i = 0; i < arrows.length; i++) {
            arrows[i].classList.remove("arrow-cooldown");
        }
        clearTimeout(arrowCooldownTimeout);
    }, ARROW_COOLDOWN_MS);
}
...
/* =========================================================
   TOOLTIP HOVER SOUND — plays "UiCursor" the instant the pointer
   enters anything that shows a hover tooltip. Uses capturing
   mouseenter/mouseleave (delegation), so it also works for tooltip
   elements built dynamically after load (Spoiler Log rows, Credits,
   Downloads, etc.). Matches native title="" tooltips, data-tooltip=""
   attributes, and any element using a "tooltip" class/wrapper.
   Adjust TOOLTIP_HOVER_SELECTOR if a new tooltip pattern is added.
   ========================================================= */
var TOOLTIP_HOVER_SELECTOR = '[title], [data-tooltip], [class*="tooltip"]';
var lastTooltipHoverEl = null;

document.addEventListener("mouseenter", function(e) {
    var el = e.target.closest ? e.target.closest(TOOLTIP_HOVER_SELECTOR) : null;
    if (el && el !== lastTooltipHoverEl) {
        lastTooltipHoverEl = el;
        wsnd.play("UiCursor");
    }
}, true);
...
document.addEventListener("mouseleave", function(e) {
    var el = e.target.closest ? e.target.closest(TOOLTIP_HOVER_SELECTOR) : null;
    if (el && el === lastTooltipHoverEl) {
        lastTooltipHoverEl = null;
    }
}, true);

/* =========================================================
   EXIT / "X OUT" SOUND — plays "UiBack" when the page is actually
   being left (closed, navigated away from, or the NX app is exited),
   as opposed to just losing focus. pagehide covers both a normal
   unload and bfcache navigations; the persisted check avoids firing
   twice on browsers that both bfcache and fully unload the page.
   ========================================================= */
